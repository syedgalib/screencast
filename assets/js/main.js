...
(function(module) {
  ...
  function bytesToSize(bytes) {
    ...
    var i = parseInt(Math.floor(Math.log(bytes) / Math.log(k2)), 10);
    return (bytes / Math.pow(k2, i)).toPrecision(3) + " " + sizes[i];
  }
  ...
  function getTracks(stream, kind) {
    ...
    return stream.getTracks().filter(function(t2) {
      return t2.kind === (kind || "audio");
    });
  }
  ...
  function StereoAudioRecorder(mediaStream, config) {
    ...
    function mergeLeftRightBuffers(config2, callback) {
      function mergeAudioBuffers(config3, cb2) {
        ...
        function interleave(leftChannel, rightChannel) {
          ...
          var inputIndex = 0;
          for (var index3 = 0; index3 < length; ) {
            result[index3++] = leftChannel[inputIndex];
            result[index3++] = rightChannel[inputIndex];
            inputIndex++;
          }
          return result;
        }
        ...
        var volume = 1;
        for (var i = 0; i < lng; i++) {
          view.setInt16(index2, interleaved[i] * (32767 * volume), true);
          index2 += 2;
        }
        if (cb2) {
          return cb2({
  ...
  function WhammyRecorder(mediaStream, config) {
    ...
    function dropBlackFrames(_frames, _framesToCheck, _pixTolerance, _frameTolerance, callback) {
      ...
      asyncLoop({
        ...
        functionToLoop: function(loop, f2) {
    ...
    function whammyInWebWorker(frames) {
      ...
      function parseRIFF(string) {
        ...
        while (offset < string.length) {
          var id2 = string.substr(offset, 4);
          var len = getStrLength(string, offset);
          var data = string.substr(offset + 4 + 4, len);
          offset += 4 + 4 + len;
          chunks[id2] = chunks[id2] || [];
          if (id2 === "RIFF" || id2 === "LIST") {
            chunks[id2].push(parseRIFF(data));
          } else {
            chunks[id2].push(data);
          }
        }
        return chunks;
      }
  ...
  var DiskStorage = {
    ...
    init: function() {
      ...
      var dbVersion = 1;
      var dbName = this.dbName || location.href.replace(/\/|:|#|%|\.|\[|\]/g, ""), db2;
      var request = indexedDB.open(dbName, dbVersion);
      function createObjectStore(dataBase) {
        dataBase.createObjectStore(self2.dataStoreName);
      }
      function putInDB() {
        var transaction = db2.transaction([self2.dataStoreName], "readwrite");
        if (self2.videoBlob) {
      ...
      request.onsuccess = function() {
        db2 = request.result;
        db2.onerror = self2.onError;
        if (db2.setVersion) {
          if (db2.version !== dbVersion) {
            var setVersion = db2.setVersion(dbVersion);
            setVersion.onsuccess = function() {
              createObjectStore(db2);
              putInDB();
            };
          } else {
            putInDB();
  ...
  function MultiStreamsMixer(arrayOfMediaStreams, elementClass) {
    ...
    function drawImage(video, idx) {
      ...
      context.drawImage(video, x2, y2, width, height);
      if (typeof video.stream.onRender === "function") {
        video.stream.onRender(context, x2, y2, width, height, idx);
      }
    }
    function getMixedStream() {
      ...
      if (mixedAudioStream) {
        mixedAudioStream.getTracks().filter(function(t2) {
          return t2.kind === "audio";
    ...
    function getMixedVideoStream() {
      ...
      capturedStream.getTracks().filter(function(t2) {
        return t2.kind === "video";
      }).forEach(function(track) {
    ...
    function getMixedAudioStream() {
      ...
      arrayOfMediaStreams.forEach(function(stream) {
        if (!stream.getTracks().filter(function(t2) {
          return t2.kind === "audio";
        }).length) {
          return;
        }
    ...
    this.appendStreams = function(streams) {
      ...
      streams.forEach(function(stream) {
        ...
        if (stream.getTracks().filter(function(t2) {
          ...
          video.stream = stream;
          videos.push(video);
          newStream.addTrack(stream.getTracks().filter(function(t2) {
            return t2.kind === "video";
          })[0]);
        }
        if (stream.getTracks().filter(function(t2) {
          ...
          self2.audioDestination = self2.audioContext.createMediaStreamDestination();
          audioSource.connect(self2.audioDestination);
          newStream.addTrack(self2.audioDestination.stream.getTracks().filter(function(t2) {
            return t2.kind === "audio";
          })[0]);
        }
      ...
      return new ReadableStream({
        start: function(controller) {
          ...
          video.onplaying = function() {
            ...
            var cameraTimer = setInterval(function f2() {
              if (finished) {
    ...
    function startRecording(stream2, buffer) {
      if (!config.workerPath && !buffer) {
        ...
        fetch(
          ...
          r2.arrayBuffer().then(function(buffer2) {
...
function getLabel(resulation) {
  const hd2 = resulation.hdKey ? ` (${resulation.hdKey})` : "";
  return `${resulation.height}p${hd2}: ${resulation.width}x${resulation.height}`;
}
...
Object.keys(resolutions).map(parseOption).reverse();
function formatTimeAsClock(timeInSecond) {
  return typeof timeInSecond === "number" ? new Date(timeInSecond * 1e3).toISOString().substring(14, 19) : "00:00";
}
function useVideoRecorder(config) {
  ...
  const [isRecordingPaused, setIsRecordingPaused] = reactExports.useState(false);
  const [permissionDenied, setPermissionDenied] = reactExports.useState(null);
  const [recordedTimeInSecond, setRecordedTimeInSecond] = reactExports.useState(0);
  const [recordedBlob, setRecordedBlob] = reactExports.useState(null);
  const [recordedURL, setRecordedURL] = reactExports.useState("");
  const [recordingIsGoingToStopSoon, setRecordingIsGoingToStopSoon] = reactExports.useState(false);
  const recorderRef = reactExports.useRef();
  const streamRef = reactExports.useRef();
  const recordingTimerRef = reactExports.useRef();
  const videoStreamRef = reactExports.useRef();
  ...
  async function hasPermission() {
    try {
      ...
      const status = microphonePermission.state === "granted";
      return status;
    } catch (_) {
      return false;
    }
  ...
  async function setupStream() {
    try {
      ...
      if (videoStreamRef.current.srcObject) {
        videoStreamRef.current.srcObject.getVideoTracks().forEach((track) => {
          track.stop();
...
function useScreenRecorder(config) {
  ...
  const recordingTimerRef = reactExports.useRef();
  const recorderRef = reactExports.useRef();
  const screenStreamRef = reactExports.useRef();
  const audioStreamRef = reactExports.useRef();
  const videoStreamRef = reactExports.useRef();
  const [recordingIsGoingToStopSoon, setRecordingIsGoingToStopSoon] = reactExports.useState(false);
  reactExports.useEffect(() => {
    if (!config.maxRecordLength) {
...
function App() {
  const [isStreamingVideo, setIsStreamingVideo] = reactExports.useState(false);
  const {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "box-container", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "actions", children: [
      ...
      !isScreenRecording && !isVideoRecording && isStreamingVideo && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", onClick: startOrStopVideoRecord, children: "Start Video Recording" }),
      isVideoRecording && /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "btn btn-danger", onClick: startOrStopVideoRecord, children: [
        "Stop Video Recording ( ",
        getRecordedVideoTimeAsClock(),
        " )"
      ] }),
